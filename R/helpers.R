...
getlinks <- function(x, df, xid, dfid, dfvar){
  ...
  colnames(x2) <- c(xid, "x", "y")
  df <- df[, c(dfid,dfvar)]
  colnames(df) <- c("i","j","fij")
  df <- df[!df$i == df$j,]
  
  link <- merge(df, x2, by.x = dfid[2], by.y = xid, all.x = TRUE)
  link <- merge(link, x2, by.x = dfid[1], by.y = xid, all.x = TRUE)
...
getdots <- function(x, xid, k = NULL){
  ...
  circles = circles[,c(xid, "r","geometry")]
  return(circles)
}
